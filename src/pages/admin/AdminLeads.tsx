...
import { Search, Download, ChevronLeft, ChevronRight, Calendar, Flag, Trash2 } from "lucide-react";
...
export default function AdminLeads() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-leads", search, stageFilter, sourceFilter, page],
    queryFn: async () => {
      let query = supabase
        .from("leads")
        .select("*", { count: "exact" })
  ...
  const { data: sourcesData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const unique = [...new Set((data || []).map(d => d.source_page).filter(Boolean))].sort();
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{data?.total || 0} active leads</p>
          </div>
        ...
        <div className="grid grid-cols-5 gap-2">
          {Object.entries(stageLabels).map(([key, label]) => {
            ...
            return (
              <Card key={key} className="cursor-pointer hover:border-primary" onClick={() => { setStageFilter(key); setPage(0); }}>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Select value={sourceFilter} onValueChange={v => { setSourceFilter(v); setPage(0); }}>
                ...
                <SelectContent>
                  ...
                  {(sourcesData || []).map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </CardHeader>
